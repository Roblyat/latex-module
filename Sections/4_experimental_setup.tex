...
This chapter specifies the experimental protocol used to evaluate the proposed two-stage DeLaN+LSTM pipeline on the IEEE DataPort collaborative-robot trajectories in the motor-current domain.
All experiments operate on trajectory logs of joint positions $\mathbf{q}$, joint velocities $\dot{\mathbf{q}}$, and per-joint motor currents $\mathbf{i}_{\mathrm{motor}}$ sampled at approximately $100\,\mathrm{Hz}$; joint accelerations $\ddot{\mathbf{q}}$ are derived from the recorded state after per-trajectory preprocessing.
To prevent leakage between trajectories, all train/validation/test splits are constructed at the trajectory level, and performance is reported as per-joint motor-current RMSE in $\mathrm{A}$, aggregated across seeds where applicable.
Two complementary studies are considered: a learning-curve sweep (K-domination) that varies the number of available trajectories $K$ to quantify optimisation stability and generalisation in both stages, and a best-model study at fixed $K$ that selects robust DeLaN and LSTM configurations based on validation performance before evaluating on the held-out test split.

\section{DeLaN + LSTM - Learning Curve Stroy}

This learning-curve study evaluates the two-stage pipeline as a function of the number of available trajectories $K$.
As $K$ increases, both the DeLaN backbone and the residual LSTM train more consistently across dataset seeds, yielding lower and more uniform per-joint motor-current errors; the residual LSTM provides an additional reduction of the remaining modelling error once the DeLaN baseline has stabilised.
...
The resulting DeLaN-ready dataset is constructed as described in Section~\ref{sec:data_preprocessing}.
...
trajectories (trajectories are never split across subsets).
...
trajectory.
Stage~2 uses a history length $H=100$ and feature mode \texttt{full}.
...
by which trajectories end up in the train/validation/test split (dataset seed) and the
...
motor-current RMSE (Figure~\ref{fig:kdom_delan_rmse_progress_by_k}).
...
hyperparameters should reduce seed sensitivity (tight IQR with few or no catastrophic runs),
reach low validation error quickly and consistently (stable early-stopping behaviour) and
generalise such that the validation-based ranking correlates with test performance.
...
median validation RMSE versus IQR of validation RMSE and median validation RMSE versus
median test RMSE.
...
The baseline preset is chosen to match the current best-practice DeLaN configuration reported in~\cite{Q4_2_lutter2023combiningphysicsdeeplearning} (\texttt{lutter\_like\_256}), while the remaining presets vary a single axis (capacity, regularisation, or step size) to probe stability and generalisation.
\begin{enumerate}
    \item \textbf{Baseline (Lutter-like):} SoftPlus, batch size $1024$, learning rate $10^{-4}$, weight decay $10^{-5}$, width $256$, depth $2$ (\texttt{lutter\_like\_256}).
    \item \textbf{Smaller capacity:} baseline with width $128$, depth $2$ (\texttt{lutter\_like\_128}); tests whether the $256$-wide model is necessary or whether a smaller network improves stability.
    \item \textbf{Deeper network:} baseline with width $256$, depth $3$ (\texttt{lutter\_like\_256\_d3}); tests whether additional depth improves representation at the expense of seed stability.
    \item \textbf{More regularisation:} baseline with weight decay $10^{-4}$ (\texttt{lutter\_like\_256\_wd1e4}); tests whether stronger regularisation reduces seed variance and improves generalisation.
    \item \textbf{Lower step size:} baseline with learning rate $5\cdot 10^{-5}$ (\texttt{lutter\_like\_256\_lr5e5}); tests whether more conservative optimisation improves numerical stability versus underfitting.
\end{enumerate}
...
\begin{algorithm}[H]
\caption{Best-model selection for the DeLaN}\label{alg:best_model_approach}
\begin{algorithmic}
\Require 4th-order Butterworth low-pass filter with cutoff frequency $f_c = 10\,\mathrm{Hz}$
\Require \texttt{UR3\_Load0Dataset} training pool with $50{,}000$ samples, split into $(70\%,10\%,20\%)$ train/validation/test subsets for model selection
...
The goal is to choose a configuration $(f,H)$ and corresponding checkpoints such that the end-to-end prediction error of the combined motor-current model
\begin{equation}
...
\end{equation}
is minimised on held-out trajectories, while remaining robust to the dataset split induced by the dataset seed $d$ and the LSTM initialisation seed.
Since the LSTM operates on history windows, evaluation is performed only on indices with valid context ($k\geq H-1$).

Algorithm~\ref{alg:lstm_best_model_approach} sweeps three window lengths $H\in\{50,100,150\}$ and four feature modes $f$.
The feature modes are designed to isolate whether the residual motor-current dynamics are primarily explained by kinematics, by the DeLaN current baseline, or by their combination:
\begin{description}
    \item[\texttt{full}] $(\mathbf{q},\dot{\mathbf{q}},\ddot{\mathbf{q}},\hat{\mathbf{i}}_{\mathrm{DeLaN}})$; tests whether the residual benefits from both kinematics and the DeLaN baseline prediction.
    \item[\texttt{i\_hat}] $(\hat{\mathbf{i}}_{\mathrm{DeLaN}})$; tests whether the residual is largely a systematic correction correlated with the DeLaN prediction alone.
    \item[\texttt{state}] $(\mathbf{q},\dot{\mathbf{q}},\ddot{\mathbf{q}})$; tests whether kinematics alone is sufficient to explain the residual, without access to the DeLaN baseline.
    \item[\texttt{state\_ihat}] $(\dot{\mathbf{q}},\ddot{\mathbf{q}},\hat{\mathbf{i}}_{\mathrm{DeLaN}})$; tests whether absolute position can be omitted while retaining velocity/acceleration information conditioned on the DeLaN baseline prediction.
\end{description}
