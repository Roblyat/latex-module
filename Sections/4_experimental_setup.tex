...
Where expressions are written in terms of motor torque, they are understood via the linear DC motor relation $\boldsymbol{\tau}_{\mathrm{motor}} = k_t\,\mathbf{i}_{\mathrm{motor}}$ introduced earlier; consequently, all reported errors in this chapter are expressed in $\mathrm{A}$ (and can be mapped to torque units by multiplication with $k_t$ when needed).
...
torque RMSE (Figure~\ref{fig:kdom_delan_rmse_progress_by_k}). \textbf{main objective best DelaN -> best LSTM here}
...
\ForAll{$h \in \mathcal{H}_{\mathrm{DeLaN}}$}
    ...
    \ForAll{$d \in \mathcal{D}$}
        ...
        \ForAll{$s \in \mathcal{S}_{\mathrm{DeLaN}}$}
            ...
            \Else
                \State Record curves $\mathrm{train\_loss}(e), \mathrm{val\_mse}(e)$, torque RMSE over progress, torque RMSE per joint, and $\mathrm{RMSE}_{\mathrm{val}}, \mathrm{RMSE}_{\mathrm{test}}, e^\star$       
            \EndIf
            \State Append curves and run metrics to $\mathcal{C}_{\mathrm{train}}, \mathcal{C}_{\mathrm{val}}, \mathcal{R}$ \Comment{includes torque RMSE progress and per-joint traces}
        \EndFor
...
\textbf{Motivation.}
After selecting and freezing the best DeLaN checkpoint (Algorithm~\ref{alg:best_model_approach}), Stage~2 addresses the remaining modelling error by learning the residual torques
$\boldsymbol{r}_{\tau,k}=\boldsymbol{\tau}_{\mathrm{motor},k}-\hat{\boldsymbol{\tau}}_{\mathrm{DeLaN},k}$ (Eq.~\ref{eq:tau_residual}).
...
The goal is to choose a configuration $(f,H)$ and corresponding checkpoints such that the \emph{end-to-end} prediction error of the combined model
$\hat{\boldsymbol{\tau}}_{\mathrm{RG},k}=\hat{\boldsymbol{\tau}}_{\mathrm{DeLaN},k}+\hat{\boldsymbol{r}}_{\tau,k}$ (Eq.~\ref{eq:combined_torque_rg})
...
\textbf{Ablation axes.}
Algorithm~\ref{alg:lstm_best_model_approach} sweeps three window lengths $H\in\{50,100,150\}$ and four feature modes $f$ that probe whether residual dynamics are primarily explained by kinematics, by the DeLaN torque baseline, or by their combination:
\texttt{state} uses $(\mathbf{q},\dot{\mathbf{q}},\ddot{\mathbf{q}})$,
\texttt{tau\_hat} uses $\hat{\boldsymbol{\tau}}_{\mathrm{DeLaN}}$,
\texttt{full} uses $(\mathbf{q},\dot{\mathbf{q}},\ddot{\mathbf{q}},\hat{\boldsymbol{\tau}}_{\mathrm{DeLaN}})$, and
\texttt{state\_tauhat} uses $(\dot{\mathbf{q}},\ddot{\mathbf{q}},\hat{\boldsymbol{\tau}}_{\mathrm{DeLaN}})$ (dropping $\mathbf{q}$).
For each $(d,f,H)$, multiple LSTM seeds are trained with early stopping, and both residual-level metrics and combined-model torque RMSE are recorded.

\textbf{Stability-aware ranking across seeds.}
To select a configuration that is both accurate and reliable, we rank $(f,H)$ by a robustness-aware score based on the combined-model torque RMSE:
...
\begin{algorithmic}
\Require Dataset seeds $\mathcal{D}$, feature modes $\mathcal{F}=\{\texttt{full},\texttt{state},\texttt{tau\_hat},\texttt{state\_tauhat}\}$, window lengths $\mathcal{H}=\{50,100,150\}$
