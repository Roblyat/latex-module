...
Subsequent chapters present the State of the Art, the proposed methods, the experimental evaluation, a discussion of the results and their implications, and an outlook on future research directions.
    ...
    Next is: use case, problem description (mathematical background), problem statement, contribution.
    Then state of the art.

    \section{Problem Statement}
    The subsection \emph{Kinematic and Dynamic Background of Robot Manipulation and Environment Interaction} analyses why endowing a robotic manipulator with awareness of its own dynamics, payload, and tools is mathematically demanding and cannot be
    ...
    in the literature, thereby motivating the contribution of this work.
        ...
        \subsection{Kinematic and Dynamic Background of Robot Manipulation and Environment Interaction}
        ...
        where $Y(\cdot)$ is the $6\times 10$ Newton-Euler regressor matrix defined in the Appendix. It is linear in the inertial parameter vector $\boldsymbol{\phi}_{\mathrm{eff}}$, but depends nonlinearly on the motion variables $(\mathbf{a},\boldsymbol{\alpha},\boldsymbol{\omega})$. The terms $\vec{F}_{\mathrm{bias}}$ and $\vec{n}$ denote sensor bias and noise, respectively.\footnote{All experiments in this work are simulation-based; in the subsequent method formulation, sensor bias and noise are therefore neglected and \eqref{eq:F_measured_regressor} is used without $\vec{F}_{\mathrm{bias}}$ and $\vec{n}$.}
        ...
        \begin{itemize}
        \item \textbf{No unified dynamic representation of robot, tool and payload.}
        Classical methods typically treat robot RDPI in joint or motor space, PDPI in the FT frame, and interaction-force estimation via observers as largely separate problems~\cite{Q1_1_fast_inertial_id_cobots,Q1_5_payload_estimation_compensation_2025,Q1_6_hu2025_fdrdi,Q1_7_10947736,Q1_13_dynamic_model_id_swevers2007,Q1_3_external_torque_smo,Q1_4_contact_force_kf,Q1_12_han_finite_time_observer,Q1_14_long2022sliding_momentum_observer,Q1_17_liu2021sensorless_dob_nn} %\cite{Q1_1, Q1_5, Q1_6, Q1_7, Q1_13, Q1_3, Q1_4, Q1_12, Q1_14, Q1_17} 
        GP-based approaches lump robot, tool, payload, friction and contact into a single disturbance term on top of a nominal RBD~\cite{Q2_1_contact_force_gp_observer,Q2_2_contact_force_gpadkf,Q2_3_contact_detection_gp,Q2_4_GIACOMUZZO20231584},
        and deep sequence models either focus on contact / torque estimation or on payload identification, but not both in a single model~\cite{Q3_1_tao_bll,Q3_2_encoder_attention_payload,Q3_3_lstm_force_estimation,Q3_4_asgrnn_force_observer,Q3_5_contact_localization_cnn,Q3_6_payload_id_incremental_ensemble,Q3_7_payload_id_online_ensemble,Q3_8_payload_id_catastrophic_forgetting}. %~\cite{Q3_1, Q3_2, Q3_3, Q3_4, Q3_5 ,Q3_6, Q3_7, Q3_8}
        Physics-informed networks provide accurate robot dynamics but assume a fixed tool/payload and contact-free operation~\cite{Q4_1_extended_delan_motor,Q4_2_lutter2023combiningphysicsdeeplearning,Q4_3_residual_pinns_dynamics_id,Q4_4_10729277,Q4_5_10305255} %\cite{Q4_1, Q4_2, Q4_3, Q4_4, Q4_5}
        None of these lines yields a compact, measurement-frame representation of the \emph{effective} rigid body (robot+tool+payload) together with its interaction wrench.
        ...
        Q3 and Q4 introduce powerful black-box or PINN components, but again focus on reducing torque/current residuals rather than producing explicit, structured estimates of friction and interaction
        ...
        Overall, the current SoA provides strong ingredients—classical LS/NE regressors, GP residuals, sequence models and physics-informed networks—but does not yet offer a compact, measurement-frame model that endows a
        manipulator with unified, robust dynamic awareness of its own robot-tool-payload system and its interaction forces under realistic cobot operating conditions.

    \section{Aim of Work}

    The SoA review suggests that the strongest results for robot inverse dynamics and friction modelling are obtained when \emph{physics-structured} networks such as DeLaN/PINN are combined with data-driven residual learners.
    Extended DeLaN models can capture motor couplings and current–torque relations with high accuracy from encoder and motor data alone~\cite{Q4_1_extended_delan_motor,Q4_2_lutter2023combiningphysicsdeeplearning},
    while recent PINN-based approaches demonstrate that adding a learned residual (e.g.\ TCN) on top of a structured dynamics model yields state-of-the-art joint-torque prediction and friction compensation on industrial
    robots~\cite{Q4_3_residual_pinns_dynamics_id,Q4_4_10729277}.
    These works, however, focus purely on RDPI for a fixed robot-tool configuration and do not provide an explicit, measurement-frame model of the effective rigid body (robot+tool+payload) or its interaction wrench.

    The aim of this thesis is to build on these insights and develop a \emph{physics-informed, sequence-model-based inverse-dynamics architecture} that serves as a strong basis for tool/gripper compensation and, ultimately,
    payload dynamic parameter identification.
    Concretely, the work pursues the following objectives:
    \begin{itemize}
    \item Use a DeLaN-style structured network to learn the nominal robot inverse dynamics from joint states and motor currents, including joint friction and drive-train effects, following the ideas
          of~\cite{Q4_1_extended_delan_motor,Q4_2_lutter2023combiningphysicsdeeplearning,Q4_4_10729277}.
            The model is trained \emph{offline} on carefully designed excitation trajectories (in simulation in this thesis), and then deployed for real-time torque prediction during operation.
    \item Augment this structured model with a recurrent sequence model (LSTM) that takes joint histories and DeLaN residuals as input and learns to predict the remaining flange wrench in the measurement frame.
            This separates the well-modelled robot base dynamics from the effective rigid-body contribution of tool and gripper, providing a compact representation of $\boldsymbol{\phi}_{\mathrm{eff}}$–induced effects that
            is consistent with the kinematic and Newton-Euler background developed in Section~\ref{sec:problem_statement}.
    \item Evaluate the resulting DeLaN+LSTM architecture as a foundation for PDPI: first by quantifying its ability to compensate tool/gripper dynamics and predict joint torques across diverse trajectories, then by analysing
          how accurately the inferred effective wrench tracks changes in attached loads.
    \end{itemize}

    By combining a physics-informed inverse-dynamics backbone with a sequence model trained offline but executed online, this work aims to move from the fragmented, calibration-heavy SoA towards a unified and practically
    deployable notion of dynamic awareness:
    a robot that can reliably predict its own joint torques and separate its intrinsic dynamics from tool and payload effects in the measurement frame, under cobot-typical operating conditions.
