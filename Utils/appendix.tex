...
Following \cite{Q4_6_HU2026103093}, the symmetric positive-definite inertia
matrix $\mathbf{M}_{\boldsymbol{\theta}}(\mathbf{q}) \in \mathbb{R}^{n\times n}$
is obtained from a learned Cholesky factor.
...
In practice, all three MLPs ($\mathbf{L}_o$, $\mathbf{L}_d$, and the potential
network below) use sinusoidal activation functions as suggested
by~\cite{Q4_6_HU2026103093}, since for revolute manipulators the dynamic
terms can be written as linear combinations of $\sin(\cdot)$ and
$\cos(\cdot)$.

\section{Potential and gravity subnetworks}

The conservative part of the dynamics is encoded by a learned
Lagrangian
\begin{equation}
...
Using automatic differentiation we obtain the gravity vector as the
...
\begin{equation}
  ...
  \mathbf{g}(\mathbf{q}).
\end{equation}
This is equivalent, up to the choice of parameterisation, to the gravity
MLP $\hat{\mathbf{g}}(\mathbf{q};\boldsymbol{\psi}_g)$ used
in~\cite{Q4_6_HU2026103093}, but keeps the explicit Lagrangian structure.
...
$\mathbf{C}_{\boldsymbol{\theta}}(\mathbf{q},\dot{\mathbf{q}})\dot{\mathbf{q}}$
obtained from $\mathcal{L}_{\boldsymbol{\theta}}$ by the
Euler–Lagrange equations.

\section{Friction subnetwork}

Following the improved DeLaN model in~\cite{Q4_6_HU2026103093},
joint friction is modelled in a physically interpretable way by a
Coulomb–viscous law,
\begin{equation}
...
\end{equation}
where $\mathbf{f}_c,\mathbf{f}_v \in \mathbb{R}^n$ collect the
joint-wise Coulomb and viscous friction coefficients and
$\mathrm{sgn}(\cdot)$ is applied element-wise.
Equation~\eqref{eq:app_coulomb_viscous} corresponds directly to
Eqs.~(13)–(15) in~\cite{Q4_6_HU2026103093}, where
\[
  \tau_f = f_c \,\mathrm{sgn}(\dot{q}) + f_v \dot{q}
\]
is added to the DeLaN prediction.%
\footnote{In preliminary experiments we found that replacing
the analytic Coulomb–viscous form by a small MLP
$\boldsymbol{\tau}_{\mathrm{fric}} = f_{\mathrm{fric}}([\dot{\mathbf{q}},\mathrm{sgn}(\dot{\mathbf{q}})];\boldsymbol{\psi})$
slightly improves training loss but makes the friction parameters harder
to interpret and more prone to absorbing other modelling errors. Given
that history-dependent and stick–slip effects are explicitly handled by
the LSTM residual model in Stage~2, we retain the simpler Coulomb–viscous
structure in Stage~1.}

\section{Resulting DeLaN inverse-dynamics map}

Combining the inertia, potential/gravity and friction subnetworks, the
DeLaN inverse-dynamics model used in this thesis is
\begin{equation}
...
\end{equation}
which is trained in Stage~1 by minimising the loss
\begin{equation}
